...
public:
    ...
    */
    ...
    /// @param rT_Min Minimun t value
    /// @param rT_Max Maximum t value, and closest so far
    ...
    bool hit(const ray &r, double rT_Min, double rT_Max, hit_record &hitRecord) const override
    {
        hit_record temporalRecord;
        bool hitSomething = false;
        auto closestSoFar = rT_Max;

        for (const auto &object : objects)
        {
            if (object->hit(r, rT_Min, closestSoFar, temporalRecord))
            {
                hitSomething = true;
                /*
